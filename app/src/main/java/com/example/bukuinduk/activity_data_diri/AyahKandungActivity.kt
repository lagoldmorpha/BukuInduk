...
import com.example.bukuinduk.databinding.ActivityAyahKandungBinding
...
class AyahKandungActivity : AppCompatActivity() {
    lateinit var binding: ActivityAyahKandungBinding
    var tanggalLahir: String? = null
    val dataStatus = listOf("Status", "masih hidup", "meninggal")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityAyahKandungBinding.inflate(layoutInflater)
        ...
        binding.spStatus.adapter = ArrayAdapter(this, R.layout.layout_custom_spinners, R.id.customSpinnerItem, dataStatus)
        ...
        if(id != 0) {
            RunRetrofit().GetOneSiswa("Bearer $token", id).enqueue(object : Callback<SiswaByID> {
                override fun onResponse(call: Call<SiswaByID>, response: Response<SiswaByID>) {
                    response.body()?.let {
                        val item = it.ayahKandung as AyahKandung
        ...
        binding.btnSimpan.setOnClickListener {
            if (isFormValid()) {
                val data = AyahKandung(
                    agama = binding.etAgama.text.toString(),
                    status = dataStatus[binding.spStatus.selectedItemPosition],
                ...
                if (id != 0) {
                    RunRetrofit().UpdateDatadiri("Bearer $token", UserDetail( ayahKandung = data), id).enqueue(object : Callback<Any>{
                    ...
                    LoginSiswaActivity.dataRequestCreateSiswa.ayahKandung = data
                    startActivity(Intent(this, IbuKandungActivity::class.java))
                    finish()
                }
            }
        }
